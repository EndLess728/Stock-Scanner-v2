...
- Head (H)            — swing high, higher than LS and RS
- Right shoulder (RS) — swing high, similar height to LS
- Neckline            — line through the two intervening swing lows
...
def detect_head_and_shoulders(series: CandleSeries, config: Dict[str, Any]) -> List[Pattern]:
    ...
    for i in range(len(swings) - 5, len(swings) - 4):
        ...
        if [s.kind for s in window] != [
            ...
            continue
        ls, lo1, head, lo2, rs = window
        if head.price <= ls.price or head.price <= rs.price:
            continue
        symmetry = abs(ls.price - rs.price) / max(head.price, 1e-9)
        if symmetry > tol:
            continue

        slope, intercept = fit_line(
            [(lo1.index_in_series, lo1.price), (lo2.index_in_series, lo2.price)]
        )
        ...
        out.append(
            Pattern(
                ...
                metadata={
                    ...
                    "reference_high": head.price,
                    "reference_low": min(lo1.price, lo2.price),
                },
