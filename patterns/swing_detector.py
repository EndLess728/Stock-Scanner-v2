...
class SwingDetector:
    ...
    def detect(self, series: CandleSeries) -> list[SwingPoint]:
        ...
        for i in range(r, n - r):
            ...
            if is_high and abs(mid.high - prev_price) / max(prev_price, 1e-9) >= self.sensitivity:
                swings.append(
                    SwingPoint(
                        timestamp=mid.start,
                        price=mid.high,
                ...
                prev_price = mid.high
            elif is_low and abs(mid.low - prev_price) / max(prev_price, 1e-9) >= self.sensitivity:
                swings.append(
                    SwingPoint(
                        timestamp=mid.start,
                        price=mid.low,
